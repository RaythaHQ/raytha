...
public class RaythaFunctionConfiguration : IRaythaFunctionConfiguration
{
    public RaythaFunctionConfiguration(IConfiguration configuration)
    {
        ...
        ExecuteTimeout = TimeSpan.FromMilliseconds(
            Convert.ToInt32(configuration["RAYTHA_FUNCTIONS_TIMEOUT"] ?? "10000")
        );
        QueueTimeout = TimeSpan.FromMilliseconds(
            Convert.ToInt32(configuration["RAYTHA_FUNCTIONS_QUEUE_TIMEOUT"] ?? "10000")
        );
    }
