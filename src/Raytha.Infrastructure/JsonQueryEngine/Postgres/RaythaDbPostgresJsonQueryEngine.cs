...
internal class RaythaDbPostgresJsonQueryEngine
        ...
        IRaythaDbJsonQueryEngine
{
    ...
    public RaythaDbPostgresJsonQueryEngine(
    ...
    protected override SqlQueryBuilder PrepareContentItemsDataSelect(SqlQueryBuilder sqlBuilder)
    {
        ...
        {
            sqlBuilder.Select(
                $"{RawSqlColumn.SOURCE_ITEM_COLUMN_NAME}.\"{column.Name}\" as \"{RawSqlColumn.SOURCE_ITEM_COLUMN_NAME}_{column.Name}\""
            );
        }
        foreach (var column in RawSqlColumn.RouteColumns())
        {
            sqlBuilder.Select(
                $"{RawSqlColumn.ROUTE_COLUMN_NAME}.\"{column.Name}\" as \"{RawSqlColumn.ROUTE_COLUMN_NAME}_{column.Name}\""
            );
        }
        foreach (var column in RawSqlColumn.UserColumns())
        {
            sqlBuilder.Select(
                $"{RawSqlColumn.SOURCE_CREATED_BY_COLUMN_NAME}.\"{column.Name}\" as \"{RawSqlColumn.SOURCE_CREATED_BY_COLUMN_NAME}_{column.Name}\""
            );
        }
        foreach (var column in RawSqlColumn.UserColumns())
        {
            sqlBuilder.Select(
                $"{RawSqlColumn.SOURCE_MODIFIED_BY_COLUMN_NAME}.\"{column.Name}\" as \"{RawSqlColumn.SOURCE_MODIFIED_BY_COLUMN_NAME}_{column.Name}\""
            );
        }
        ...
        foreach (var item in OneToOneRelationshipFields)
        {
            ...
            {
                sqlBuilder.Select(
                    $"{RawSqlColumn.RELATED_ITEM_COLUMN_NAME}_{index}.\"{column.Name}\" as \"{RawSqlColumn.RELATED_ITEM_COLUMN_NAME}_{index}_{column.Name}\""
                );
            }
            foreach (var column in RawSqlColumn.RouteColumns())
            {
                sqlBuilder.Select(
                    $"{RawSqlColumn.RELATED_ROUTE_COLUMN_NAME}_{index}.\"{column.Name}\" as \"{RawSqlColumn.RELATED_ROUTE_COLUMN_NAME}_{index}_{column.Name}\""
                );
            }
            foreach (var column in RawSqlColumn.UserColumns())
            {
                sqlBuilder.Select(
                    $"{RawSqlColumn.RELATED_CREATED_BY_COLUMN_NAME}_{index}.\"{column.Name}\" as \"{RawSqlColumn.RELATED_CREATED_BY_COLUMN_NAME}_{index}_{column.Name}\""
                );
            }
            foreach (var column in RawSqlColumn.UserColumns())
            {
                sqlBuilder.Select(
                    $"{RawSqlColumn.RELATED_MODIFIED_BY_COLUMN_NAME}_{index}.\"{column.Name}\" as \"{RawSqlColumn.RELATED_MODIFIED_BY_COLUMN_NAME}_{index}_{column.Name}\""
                );
            }
        }
