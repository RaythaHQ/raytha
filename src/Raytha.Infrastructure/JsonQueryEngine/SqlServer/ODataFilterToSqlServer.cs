...
internal class ODataFilterToSqlServer : AbstractODataFilterToSql
{
    ...
    public override string GenerateSql(string filterExpression)
    {
        ...
        IEdmNavigationSource edmNavigationSource = edmModel.FindDeclaredEntitySet(
        ...
        ODataQueryOptionParser parser = new ODataQueryOptionParser(
            ...
            edmType,
            edmNavigationSource,
            odataOptionsAsDictionary
        );

        FilterClause parsedODataFilterClause = parser.ParseFilter();

        ODataFilterToSqlVisitor<object> visitor = new ODataFilterToSqlVisitor<object>(
            _contentType,
            _primaryFieldName,
            _relatedObjectFields,
            _dateTimeFormat
        );
    ...
    private class ODataFilterToSqlVisitor<TSource> : AbstractODataFilterToSqlVisitor<TSource>
        where TSource : class
    {
        public ODataFilterToSqlVisitor(
            ContentType contentType,
            string primaryFieldName,
            IEnumerable<ContentTypeField> relatedObjectFields,
            string dateTimeFormat
        ...
        public override TSource Visit(SingleValueFunctionCallNode nodeIn)
        {
            ...
            SingleValueOpenPropertyAccessNode fieldName =
                ((ConvertNode)nodeIn.Parameters.ToArray()[0]).Source
                as SingleValueOpenPropertyAccessNode;
            ...
            if (
                BuiltInContentTypeField.ReservedContentTypeFields.Any(p =>
                    p.DeveloperName == realFieldName
                )
            )
            {
                string prefix =
                    $"{RawSqlColumn.SOURCE_ITEM_COLUMN_NAME}.{realFieldName} COLLATE Latin1_General_CI_AS LIKE";
            ...
            else
            {
                ...
                {
                    var relatedObjectField = _relatedObjectFields.First(p =>
                        p.DeveloperName == chosenColumnAsCustomField.DeveloperName
                    );
                    int indexOfRelatedObject = _relatedObjectFields
                        .ToList()
                ...
                )
                {
                    if (functionName != "contains")
                        throw new NotImplementedException(
                            $"{functionName} is not an implemented function call."
                        );
            ...
            else
            {
                ...
                )
                {
                    ...
                    string relatedObjPrimaryFieldName = relatedObjectField
                        ...
                        .DeveloperName;
                    whereClause.Append(
                        chosenColumnAsCustomField.FieldType.SqlServerSingleJsonValue(
                            $"{RawSqlColumn.RELATED_ITEM_COLUMN_NAME}_{indexOfRelatedObject}",
                            RawSqlColumn.PublishedContent.Name,
                            relatedObjPrimaryFieldName
                        )
