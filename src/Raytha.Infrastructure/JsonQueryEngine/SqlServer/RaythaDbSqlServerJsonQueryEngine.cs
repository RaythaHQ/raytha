...
internal class RaythaDbSqlServerJsonQueryEngine
    : AbstractRaythaDbJsonQueryEngine,
        IRaythaDbJsonQueryEngine
{
    public readonly IRaythaDbContext _entityFramework;
    public readonly IDbConnection _db;
    public readonly ICurrentOrganization _currentOrganization;

    public RaythaDbSqlServerJsonQueryEngine(
        IRaythaDbContext entityFramework,
        IDbConnection db,
    ...
    public override ContentItem FirstOrDefault(Guid entityId)
    {
        ...
        sqlBuilder.AndWhere(
            $"{RawSqlColumn.SOURCE_ITEM_COLUMN_NAME}.{RawSqlColumn.Id.Name} = '{entityId}'"
        );
    ...
    )
    {
        ...
        var contentType = _entityFramework
        ...
        sqlBuilder.AndWhere(
            $"{RawSqlColumn.SOURCE_ITEM_COLUMN_NAME}.{RawSqlColumn.ContentTypeId.Name} = '{contentTypeId}'"
        );
    ...
    )
    {
        ...
        using (IDbTransaction transaction = _db.BeginTransaction(IsolationLevel.Snapshot))
        {
            ...
            {
                foreach (
                    var item in QueryContentItems(
                        contentTypeId,
                        searchOnColumns,
                        search,
                        filters,
                        View.DEFAULT_MAX_ITEMS_PER_PAGE,
                        pageIndex,
                        orderBy,
                        transaction: transaction
                    )
    ...
    private SqlQueryBuilder PrepareODataFilters(SqlQueryBuilder sqlBuilder, string[] filters)
    {
        ...
        var oDataToSql = new ODataFilterToSqlServer(
            ContentType,
    ...
    protected override SqlQueryBuilder PrepareContentItemsDataSelect(SqlQueryBuilder sqlBuilder)
    {
        sqlBuilder.Select(
            RawSqlColumn
                .NameAsFullColumnLabelForEnumerable(
                    RawSqlColumn.ContentItemColumns(),
        ...
        sqlBuilder.Select(
            RawSqlColumn
                .NameAsFullColumnLabelForEnumerable(
                    RawSqlColumn.RouteColumns(),
                    RawSqlColumn.ROUTE_COLUMN_NAME
                )
                .ToArray()
        );
        sqlBuilder.Select(
            RawSqlColumn
                .NameAsFullColumnLabelForEnumerable(
                    RawSqlColumn.UserColumns(),
                    RawSqlColumn.SOURCE_CREATED_BY_COLUMN_NAME
                )
                .ToArray()
        );
        sqlBuilder.Select(
        ...
        foreach (var item in OneToOneRelationshipFields)
        {
            ...
            sqlBuilder.Select(
                RawSqlColumn
                    .NameAsFullColumnLabelForEnumerable(
                        RawSqlColumn.ContentItemColumns(),
                        $"{RawSqlColumn.RELATED_ITEM_COLUMN_NAME}_{index}"
                    )
                    .ToArray()
            );
            sqlBuilder.Select(
                RawSqlColumn
                    .NameAsFullColumnLabelForEnumerable(
                        RawSqlColumn.RouteColumns(),
                        $"{RawSqlColumn.RELATED_ROUTE_COLUMN_NAME}_{index}"
                    )
                    .ToArray()
            );
            sqlBuilder.Select(
                RawSqlColumn
                    .NameAsFullColumnLabelForEnumerable(
                        RawSqlColumn.UserColumns(),
                        $"{RawSqlColumn.RELATED_CREATED_BY_COLUMN_NAME}_{index}"
                    )
                    .ToArray()
            );
            sqlBuilder.Select(
                RawSqlColumn
                    .NameAsFullColumnLabelForEnumerable(
                        RawSqlColumn.UserColumns(),
                        $"{RawSqlColumn.RELATED_MODIFIED_BY_COLUMN_NAME}_{index}"
                    )
                    .ToArray()
            );
        }
    ...
    {
        sqlBuilder.From(
            $"{RawSqlColumn.CONTENT_ITEM_TABLE_NAME} AS {RawSqlColumn.SOURCE_ITEM_COLUMN_NAME}"
        );
        foreach (var item in OneToOneRelationshipFields)
        {
            int index = OneToOneRelationshipFields.IndexOf(item);
            ...
            sqlBuilder.Join(
                $"{RawSqlColumn.USERS_TABLE_NAME} AS {RawSqlColumn.RELATED_CREATED_BY_COLUMN_NAME}_{index}",
            ...
            sqlBuilder.Join(
                $"{RawSqlColumn.USERS_TABLE_NAME} AS {RawSqlColumn.RELATED_MODIFIED_BY_COLUMN_NAME}_{index}",
            ...
            sqlBuilder.Join(
                $"{RawSqlColumn.ROUTES_TABLE_NAME} AS {RawSqlColumn.RELATED_ROUTE_COLUMN_NAME}_{index}",
        ...
        sqlBuilder.Join(
            $"{RawSqlColumn.ROUTES_TABLE_NAME} AS {RawSqlColumn.ROUTE_COLUMN_NAME} ",
            $"{RawSqlColumn.SOURCE_ITEM_COLUMN_NAME}.{RawSqlColumn.RouteId.Name} = {RawSqlColumn.ROUTE_COLUMN_NAME}.{RawSqlColumn.Id.Name}",
            joinType: "LEFT"
        );
        sqlBuilder.Join(
            $"{RawSqlColumn.USERS_TABLE_NAME} AS {RawSqlColumn.SOURCE_CREATED_BY_COLUMN_NAME}",
        ...
        sqlBuilder.Join(
            $"{RawSqlColumn.USERS_TABLE_NAME} AS {RawSqlColumn.SOURCE_MODIFIED_BY_COLUMN_NAME}",
        ...
        foreach (var column in searchOnColumns)
        {
            ...
            else
            {
                ...
                if (reservedField != null)
                {
                    ...
                    {
                        searchFilters.Add(
                            $"{RawSqlColumn.SOURCE_CREATED_BY_COLUMN_NAME}.{RawSqlColumn.FirstName.Name} COLLATE Latin1_General_CI_AS LIKE @search"
                        );
                        searchFilters.Add(
                            $"{RawSqlColumn.SOURCE_CREATED_BY_COLUMN_NAME}.{RawSqlColumn.LastName.Name} COLLATE Latin1_General_CI_AS LIKE @search"
                        );
                    }
                    else if (
                        reservedField.DeveloperName
                    ...
                    {
                        searchFilters.Add(
                            $"{RawSqlColumn.SOURCE_MODIFIED_BY_COLUMN_NAME}.{RawSqlColumn.FirstName.Name} COLLATE Latin1_General_CI_AS LIKE @search"
                        );
                        searchFilters.Add(
                            $"{RawSqlColumn.SOURCE_MODIFIED_BY_COLUMN_NAME}.{RawSqlColumn.LastName.Name} COLLATE Latin1_General_CI_AS LIKE @search"
                        );
                    }
                    else if (
                        reservedField.DeveloperName
        ...
        foreach (var orderByElement in orderBySplit)
        {
            try
            {
                ...
                if (columnAsContentTypeField != null)
                {
                    ...
                    if (
                        columnAsContentTypeField.FieldType.DeveloperName
                        == BaseFieldType.OneToOneRelationship
                    )
                    {
                        var relatedObjectField = OneToOneRelationshipFields.First(p =>
                            p.DeveloperName == columnAsContentTypeField.DeveloperName
                        );
                        int indexOfRelatedObject = OneToOneRelationshipFields
                            .ToList()
                            .IndexOf(relatedObjectField);
                        string relatedObjPrimaryFieldName = relatedObjectField
                            .ContentType.ContentTypeFields.First(p =>
                                p.Id == relatedObjectField.ContentType.PrimaryFieldId
                            )
                            .DeveloperName.ToDeveloperName();
                        sqlBuilder.OrderBy(
                            columnAsContentTypeField.FieldType.SqlServerOrderByExpression(
                ...
                else
                {
                    ...
                    if (reservedField != null)
                    {
                        ...
                        {
                            sqlBuilder.OrderBy(
                                reservedField.FieldType.SqlServerOrderByExpression(
                                    RawSqlColumn.SOURCE_ITEM_COLUMN_NAME,
                                    RawSqlColumn.PublishedContent.Name,
                                    PrimaryFieldDeveloperName,
                                    direction.DeveloperName
                        ...
                        {
                            sqlBuilder.OrderBy(
                                $"{RawSqlColumn.SOURCE_CREATED_BY_COLUMN_NAME}.{RawSqlColumn.FirstName.Name} {direction.DeveloperName}"
                            );
                        }
                        else if (
                            reservedField.DeveloperName == BuiltInContentTypeField.LastModifierUser
                        )
                        {
                            sqlBuilder.OrderBy(
                        ...
                        {
                            sqlBuilder.OrderBy(
                                $"{RawSqlColumn.SOURCE_ITEM_COLUMN_NAME}.{reservedField.DeveloperName} {direction.DeveloperName}"
                            );
                        }
                    }
                }
