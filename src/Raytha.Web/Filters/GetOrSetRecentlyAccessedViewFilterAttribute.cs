...
public class GetOrSetRecentlyAccessedViewFilterAttribute : ActionFilterAttribute
{
    ...
    public override async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
    {
        ...
        ViewDto viewDto;
        var contentType = await _mediator.Send(new GetContentTypeByDeveloperName.Query { DeveloperName = contentTypeDeveloperName });

        if (viewId == ShortGuid.Empty)
        {
            var loggedInUser = await _mediator.Send(new GetAdminById.Query { Id = _currentUser.UserId.Value });
            var lastViewed = loggedInUser.Result.RecentlyAccessedViews.FirstOrDefault(p => p.ContentTypeId == contentType.Result.Id);

            if (lastViewed != null)
            {
                try
                {
                    var response = await _mediator.Send(new GetViewById.Query { Id = lastViewed.ViewId });
                    viewDto = response.Result;
                }
                catch (NotFoundException ex)
                {
                    var response = await _mediator.Send(new GetViews.Query
                    {
        ...
        {
            var response = await _mediator.Send(new GetViewById.Query
            {
                Id = viewId.Value
            });
        ...
        if (context.Controller is Controller controller)
        {
            ...
            if (controller.ViewData.Model is IMustHaveCurrentViewForList paginationModelForCurrentView)
            {
            ...
            if (controller.ViewData.Model is IMustHaveFavoriteViewsForList paginationModelForFavorites)
            {
                var favoriteViews = await _mediator.Send(new GetFavoriteViewsForAdmin.Query
                {
                    ContentTypeId = viewDto.ContentTypeId,
                    UserId = _currentUser.UserId.Value
                });

                var favoriteViewsViewModel = favoriteViews.Result.TotalCount > 0 ? favoriteViews.Result.Items.Select(
                    p => new FavoriteViewsForList_ViewModel
                    {
                        Id = p.Id,
                        Label = p.Label
