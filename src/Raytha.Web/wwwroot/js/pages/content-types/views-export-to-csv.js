/**
 * Content Type Views - Export to CSV Page
 * Handles background task polling and progress display for CSV exports
 */
 ...
 * @param {string} config.jobId - Background job ID to track
 */
function initProgressTracker(config) {
  ...
  if (!progressBar || !statusInfo || !config.jobId) {
    return; // Not on progress view
  }

  const { pathBase, jobId } = config;
  const currentPath = window.location.pathname + '?json=true&jobId=' + encodeURIComponent(jobId);
  ...
  /**
   * Try to redirect if statusInfo contains media item JSON
...
ready(() => {
  const configElement = document.getElementById('export-progress-config');
  if (configElement) {
    const config = {
      pathBase: configElement.dataset.pathBase || '',
      jobId: configElement.dataset.jobId || ''
    };
