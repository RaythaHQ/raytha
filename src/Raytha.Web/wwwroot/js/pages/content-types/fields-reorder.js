/**
 * Content Type Fields - Reorder Page
 * Handles drag-and-drop reordering of content type fields using SortableJS
 */
...
/**
 * Initialize sortable field list
 */
function initSortable() {
  const listEl = document.querySelector('[data-sortable-list]');
  if (!listEl) return;

  const items = $$('li[data-sortable-update-url]', listEl);
  ...
  async function onReordered(evt) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        showNotification('Field order saved ✅', true);
      }
    } catch (error) {
