...
public class WebTemplatesController : BaseController
{
    ...
    > GetWebTemplates([FromQuery] GetWebTemplatesAsListItems.Query request)
    {
        var response =
            await Mediator.Send(request) as QueryResponseDto<ListResultDto<WebTemplateListItemDto>>;
        return response;
    }
