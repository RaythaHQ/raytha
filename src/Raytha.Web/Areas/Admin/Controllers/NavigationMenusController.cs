...
public class NavigationMenusController : BaseController
{
                                           ...
                                           int pageSize = 50)
    {
        ...
        var latestNavigationMenuRevisionsResponse = await Mediator.Send(new GetLatestNavigationMenuRevisions.Query());

        var items = allNavigationMenuResponse.Result.Items.Select(nm =>
        {
            var latestRevision = latestNavigationMenuRevisionsResponse.Result.FirstOrDefault(nmr => nmr.NavigationMenuId == nm.Id);
            var (lastModificationTime, lastModificationUser) = Nullable.Compare(nm.LastModificationTime, latestRevision?.CreationTime) > 0
        ...
        var viewModel = new List_ViewModel<NavigationMenusListItem_ViewModel>(items, allNavigationMenuResponse.Result.TotalCount);
    ...
    public async Task<IActionResult> Create(NavigationMenusCreate_ViewModel model)
    {
        ...
        {
            SetErrorMessage("There was an error attempting to save this menu. See the error below.", response.GetErrors());
    ...
    {
        var input = new GetNavigationMenuById.Query
        {
            Id = id,
        };
    ...
    {
        var input = new EditNavigationMenu.Command
        {
            Id = id,
            Label = model.Label,
        };
    ...
    {
        var input = new DeleteNavigationMenu.Command
        {
            Id = id,
        };
                                               ...
                                               int pageSize = 50)
    {
        ...
        var items = response.Result.Items.Select(nmr => new NavigationMenuRevisionsListItem_ViewModel
        {
            Id = nmr.Id,
            MenuItems = nmr.NavigationMenuItemsJson,
            CreationTime = CurrentOrganization.TimeZoneConverter.UtcToTimeZoneAsDateTimeFormat(nmr.CreationTime),
            CreatorUser = nmr.CreatorUser?.FullName ?? "N/A",
        });

        var viewModel = new NavigationMenuRevisionsPagination_ViewModel(items, response.Result.TotalCount, id);

        return View(viewModel);
    }

    [HttpPost]
    [Route($"{RAYTHA_ROUTE_PREFIX}/menus/edit/{{id}}/revisions/{{revisionId}}", Name = "menusrevisionsrevert")]
    public async Task<IActionResult> RevisionRevert(string id, string revisionId)
    {
        var input = new RevertNavigationMenu.Command
        {
            Id = revisionId,
        };
    ...
    {
        var input = new SetAsMainMenu.Command
        {
            Id = id,
        };
        ...
        if (response.Success)
            ...
            SetErrorMessage("There was an error setting this as the main menu.", response.GetErrors());
    ...
    public async Task<IActionResult> MenuItems(string navigationMenuId)
    {
        ...
        var response = await Mediator.Send(input);
        var items = response.Result.BuildTree<NavigationMenuItemsListItem_ViewModel>((nmi, nestedNavigationMenuItems, _) => new NavigationMenuItemsListItem_ViewModel
        {
            Id = nmi.Id,
            Label = nmi.Label,
            Url = nmi.Url,
    ...
    [Route($"{RAYTHA_ROUTE_PREFIX}/menus/edit/{{navigationMenuId}}/menu-items/create", Name = "menuitemscreate")]
    ...
    [ValidateAntiForgeryToken]
    [Route($"{RAYTHA_ROUTE_PREFIX}/menus/edit/{{navigationMenuId}}/menu-items/create", Name = "menuitemscreate")]
    public async Task<IActionResult> MenuItemsCreate(NavigationMenuItemsCreate_ViewModel model, string navigationMenuId)
    {
        ...
        if (response.Success)
        {
            ...
            return RedirectToAction(nameof(MenuItemsEdit), new { navigationMenuId, id = response.Result });
        }
        else
        {
            SetErrorMessage("There was an error attempting to save this menu item. See the error below.", response.GetErrors());

            return View(model);
        }
    }

    [Route($"{RAYTHA_ROUTE_PREFIX}/menus/edit/{{navigationMenuId}}/menu-items/edit/{{id}}", Name = "menuitemsedit")]
    public async Task<IActionResult> MenuItemsEdit(string navigationMenuId, string id)
    {
        var navigationMenuItemResponse = await Mediator.Send(new GetNavigationMenuItemById.Query
        {
            Id = id,
        });

        var allNavigationMenuItemsResponse = await Mediator.Send(new GetNavigationMenuItemsByNavigationMenuId.Query
        {
            NavigationMenuId = navigationMenuId,
        });

        var navigationMenuItems = allNavigationMenuItemsResponse.Result.ExcludeNestedNavigationMenuItems(id).Where(nmi => nmi.Id != id);
        var viewModel = new NavigationMenuItemsEdit_ViewModel
        {
            ...
            ParentNavigationMenuItemId = navigationMenuItemResponse.Result.ParentNavigationMenuItemId,
            NavigationMenuId = navigationMenuId,
    ...
    [ValidateAntiForgeryToken]
    [Route($"{RAYTHA_ROUTE_PREFIX}/menus/edit/{{navigationMenuId}}/menu-items/edit/{{id}}", Name = "menuitemsedit")]
    public async Task<IActionResult> MenuItemsEdit(NavigationMenuItemsEdit_ViewModel model, string navigationMenuId, string id)
    {
    ...
    [HttpPost]
    [Route($"{RAYTHA_ROUTE_PREFIX}/menus/edit/{{navigationMenuId}}/menu-items/delete/{{id}}", Name = "menuitemsdelete")]
    ...
    [Route($"{RAYTHA_ROUTE_PREFIX}/menus/edit/{{navigationMenuId}}/menu-items/reorder", Name = "menuitemsreorder")]
    public async Task<IActionResult> MenuItemsReorder(string navigationMenuId)
    {
        ...
        var navigationMenuItemsByParentNavigationMenuItemId = response.Result
            .GroupBy(nmi => nmi.ParentNavigationMenuItemId)
            .Where(groups => groups.Count() > 1)
            .ToDictionary(g => g.Key?.ToString() ?? string.Empty, g => g
        ...
        var viewModel = new NavigationMenuItemsReorder_VieModel
        {
            ...
            NavigationMenuItemsByParentNavigationMenuItemId = navigationMenuItemsByParentNavigationMenuItemId,
    ...
    [HttpPatch]
    [Route($"{RAYTHA_ROUTE_PREFIX}/menus/edit/{{navigationMenuId}}/menu-items/reorder/{{id}}", Name = "menuitemsreorderajax")]
