...
public class LoginController : BaseController
{
    ...
    public async Task<IActionResult> LoginWithEmailAndPassword(string returnUrl = null)
    {
        ...
        var items = response.Result.Items.Select(
            p => new LoginAuthenticationSchemeChoiceItem_ViewModel
            {
    ...
    )
    {
        ...
        else
        {
            ...
            var items = authSchemes.Result.Items.Select(
                p => new LoginAuthenticationSchemeChoiceItem_ViewModel
                {
                    DeveloperName = p.DeveloperName,
                    AuthenticationSchemeType = p.AuthenticationSchemeType,
            ...
            var viewModel = new LoginWithEmailAndPassword_ViewModel
            {
                AuthenticationSchemes = items,
            };
    ...
    [Route(
        RAYTHA_ROUTE_PREFIX + "/login/magic-link/complete/{token?}",
        Name = "adminloginmagiclinkcomplete"
    )]
    ...
    [Route(
        RAYTHA_ROUTE_PREFIX + "/login/forgot-password/complete/{token?}",
        Name = "adminforgotpasswordcomplete"
    )]
