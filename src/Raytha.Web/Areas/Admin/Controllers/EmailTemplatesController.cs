...
using Raytha.Application.Templates.Email.Queries;
using Raytha.Domain.Entities;
using Raytha.Domain.ValueObjects;
using Raytha.Web.Areas.Admin.Views.Shared.ViewModels;
using Raytha.Web.Areas.Admin.Views.Templates.Email;
...
public class EmailTemplatesController : BaseController
{
    ...
    public async Task<IActionResult> Index(string search = "", string orderBy = $"Subject {SortOrder.ASCENDING}", int pageNumber = 1, int pageSize = 50)
    {
        ...
        return View("~/Areas/Admin/Views/Templates/Email/Index.cshtml", viewModel);
    }
    ...
    public async Task<IActionResult> Revisions(string id, string orderBy = $"CreationTime {SortOrder.DESCENDING}", int pageNumber = 1, int pageSize = 50)
    {
        ...
        return View("~/Areas/Admin/Views/Templates/Email/Revisions.cshtml", viewModel);
    }
    ...
    protected Dictionary<string, IEnumerable<InsertVariableListItem_ViewModel>> GetInsertVariablesViewModel(string emailTemplate)
    {
        ...
        var emailTemplateVariables = InsertVariableTemplateFactory.From(emailTemplate).TemplateInfo.GetTemplateVariables().Select(p => new InsertVariableListItem_ViewModel
        {
            DeveloperName = p.Key,
            TemplateVariable = p.Value
        });
