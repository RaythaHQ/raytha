...
public class ContentItemUpdatedEventHandler : INotificationHandler<ContentItemUpdatedEvent>
{
    ...
    public ContentItemUpdatedEventHandler(IBackgroundTaskQueue taskQueue, IRaythaDbContext db)
    {
    ...
    )
    {
        var activeFunctions = _db.RaythaFunctions.Where(p =>
            p.IsActive
            && p.TriggerType == RaythaFunctionTriggerType.ContentItemUpdated.DeveloperName
        );
