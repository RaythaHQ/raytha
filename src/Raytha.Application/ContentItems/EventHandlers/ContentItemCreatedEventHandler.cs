...
public class ContentItemCreatedEventHandler : INotificationHandler<ContentItemCreatedEvent>
{
    ...
    public ContentItemCreatedEventHandler(IBackgroundTaskQueue taskQueue, IRaythaDbContext db)
    {
    ...
    )
    {
        var activeFunctions = _db.RaythaFunctions.Where(p =>
            p.IsActive
            && p.TriggerType == RaythaFunctionTriggerType.ContentItemCreated.DeveloperName
        );
