    ...
    public class Handler : IRequestHandler<Command, CommandResponseDto<ShortGuid>>
    {
        ...
        )
        {
            ...
            var restoredEntity = new ContentItem
            {
                ...
                IsDraft = false,
                IsPublished = false,
                Route = new Route { Path = path, ContentItemId = entity.OriginalContentItemId },
            };
            _db.ContentItems.Add(restoredEntity);
            _db.DeletedContentItems.Remove(entity);

            var webTemplateIdsFromJson = JsonSerializer.Deserialize<IEnumerable<Guid>>(
                entity.WebTemplateIdsJson
            )!;

            var webTemplateInfos = await _db
                .WebTemplates.Where(wt => webTemplateIdsFromJson.Contains(wt.Id))
            ...
            {
                var defaultWebTemplate = await _db
            ...
            await _db.WebTemplateContentItemRelations.AddRangeAsync(
                webTemplateInfos.Select(wt => new WebTemplateContentItemRelation
                {
                    Id = Guid.NewGuid(),
