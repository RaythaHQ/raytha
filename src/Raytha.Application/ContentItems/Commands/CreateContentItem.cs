                ...
                .Custom(
                    (request, context) =>
                    {
                        ...
                        if (template.ThemeId != activeThemeId)
                        {
                            context.AddFailure(
                                Constants.VALIDATION_SUMMARY,
                                "This template does not apply to the current active theme."
                            );
                            return;
                        }

                        if (
                            !template.TemplateAccessToModelDefinitions.Any(p =>
                                p.ContentTypeId == contentTypeDefinition.Id
                            )
                        )
                        {
                            context.AddFailure(
        ...
        public string GetRoutePath(dynamic content, Guid entityId, Guid contentTypeId)
        {
            ...
            string path = routePathTemplate
                ...
                .Replace(
                    ...
                    primaryField.IfNullOrEmpty((ShortGuid)entityId)
                )
