...
public record GetPagedEntitiesInputDto
{
    ...
    {
        List<GetPagedEntitiesInputDtoOrderByItem> orderByItems =
            new List<GetPagedEntitiesInputDtoOrderByItem>();
        ...
        {
            string[] propertyAndDirection = orderByItemAsString
                .Split(" ")
                .Select(p => p.Trim())
                .Where(p => !string.IsNullOrEmpty(p))
                .ToArray();
            if (
                propertyAndDirection != null
                && propertyAndDirection.Count() == 2
                && IsValidOrderByDirection(propertyAndDirection[ORDER_BY_DIRECTION_INDEX])
            )
                orderByItems.Add(
                    new GetPagedEntitiesInputDtoOrderByItem
                    {
    ...
    {
        return orderByDirection.ToLower() == SortOrder.ASCENDING
            || orderByDirection.ToLower() == SortOrder.DESCENDING;
    }
}
