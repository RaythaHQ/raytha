...
public class EditPublicSettings
{
    ...
    public class Validator : AbstractValidator<Command>
    {
        public Validator(IRaythaDbContext db)
        {
            ...
            RuleFor(x => x.MaxNumberOfItemsPerPage)
                .GreaterThan(0)
                .GreaterThanOrEqualTo(p => p.DefaultNumberOfItemsPerPage);
            RuleFor(x => x)
                .Custom(
                    (request, context) =>
                    {
    ...
    public class Handler : IRequestHandler<Command, CommandResponseDto<ShortGuid>>
    {
        ...
        {
            var entity = _db.Views.Include(p => p.Route).First(p => p.Id == request.Id.Guid);
            ...
            entity.IgnoreClientFilterAndSortQueryParams =
                request.IgnoreClientFilterAndSortQueryParams;
