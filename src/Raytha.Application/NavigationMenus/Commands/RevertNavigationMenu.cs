...
public class RevertNavigationMenu
{
    ...
    public class Handler : IRequestHandler<Command, CommandResponseDto<ShortGuid>>
    {
        ...
        {
            var navigationMenuRevision = await _db
                .NavigationMenuRevisions.Include(nmr => nmr.NavigationMenu)
            ...
            await _mediator.Send(
                new CreateNavigationMenuRevision.Command { NavigationMenuId = navigationMenu.Id },
                cancellationToken
            );

            _db.NavigationMenuItems.RemoveRange(navigationMenu.NavigationMenuItems);

            var navigationMenuItemsFromJson = JsonSerializer.Deserialize<
                ICollection<NavigationMenuItem>
            >(navigationMenuRevision.NavigationMenuItemsJson);
            await _db.NavigationMenuItems.AddRangeAsync(
                navigationMenuItemsFromJson!,
                cancellationToken
            );
