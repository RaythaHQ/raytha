...
public class CreateContentTypeField
{
    ...
    public class Validator : AbstractValidator<Command>
    {
        public Validator(IRaythaDbContext db)
        {
            ...
            RuleFor(x => x.Choices)
                .NotEmpty()
                .When(p => BaseFieldType.From(p.FieldType).HasChoices);
            RuleFor(x => x.RelatedContentTypeId)
                .NotEmpty()
                .When(p => p.FieldType == BaseFieldType.OneToOneRelationship);
            RuleFor(x => x.DeveloperName)
                .Must(p => NotReservedFieldName(p))
                .WithMessage($"Reserved word - cannot be used as a developer name");
            RuleFor(x => x)
                .Custom(
                    (request, context) =>
                    {
                        var contentTypeField = db
                            .ContentTypeFields.IgnoreQueryFilters()
                            .Where(ctf =>
                                ctf.ContentTypeId == request.ContentTypeId.Guid
                                && ctf.DeveloperName == request.DeveloperName.ToDeveloperName()
                            )
                        ...
                        if (contentTypeField != null && !contentTypeField.IsDeleted)
                        {
                            context.AddFailure(
                                "DeveloperName",
                                "Another field with that developer name already exists."
                            );
                            return;
                        }
                        ...
                        if (contentTypeField != null && contentTypeField.IsDeleted)
                        {
                            context.AddFailure(
                                "DeveloperName",
                                "A previously deleted field has already used that developer name. You must choose another one."
                            );
                            return;
                        }
                        ...
                        {
                            var choiceDeveloperNames = request.Choices.Select(p =>
                                p.DeveloperName.ToDeveloperName()
                            );
                            ...
                            if (foundDuplicates.Any())
                            {
                                context.AddFailure(
                                    "Choices",
                                    $"Your choices must have unique developer names. Duplicate developer names found: {dupes}"
                                );
                                return;
                            }
                            ...
                            if (choiceDeveloperNames.Any(p => string.IsNullOrWhiteSpace(p)))
                            {
                                context.AddFailure(
                                    "Choices",
                                    "Developer names cannot be null or empty."
                                );
                                return;
                            }
                        ...
                        if (
                            ...
                            && !db.ContentTypes.Any(p =>
                                p.Id == request.RelatedContentTypeId.Value.Guid
                            )
                        )
                        {
                            context.AddFailure(
                                "RelatedContentTypeId",
                                $"{request.RelatedContentTypeId} did not match any existing content type."
                            );
                            return;
                        }
                    }
    ...
    public class Handler : IRequestHandler<Command, CommandResponseDto<ShortGuid>>
    {
        ...
        {
            var ordinal = _db.ContentTypeFields.Count(p =>
            ...
            {
                choices = request.Choices.Select(p => new ContentTypeFieldChoice
                {
                    DeveloperName = p.DeveloperName.ToDeveloperName(),
                    Disabled = p.Disabled,
                    Label = p.Label,
                });
            }
            ...
            Guid? relatedContentTypeId =
                request.RelatedContentTypeId == ShortGuid.Empty
                || request.RelatedContentTypeId == null
                    ? null
                    : request.RelatedContentTypeId.Value.Guid;
